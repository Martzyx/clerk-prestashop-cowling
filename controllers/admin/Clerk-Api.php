...
class Clerk_Api
{
    ...
    public function addProduct($product, $product_id, $qty = 0)
    {
        try {
            ...
            if ($continue) {
                ...
                if($product_id) {
                    $Product_params['rrp'] = number_format(Product::getRRPFigure( $product_id ), 2 );

                    $roundedRate = $this->getProductRating($product_id);

                    if(is_numeric($roundedRate)) {
                        $Product_params['show_ratings_image'] = ($roundedRate >= 4) ? 1 : 0;

                        $Product_params['ratings_image'] = "/modules/netreviews/views/img/etoile".$roundedRate.".png";
                    }
                }
    ...
    private function getProductRating($product_id)
    {
        ...
        $reviews = $reviewsModel->getProductReviews($product_id);
